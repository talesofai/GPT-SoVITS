...
role_selector = gr.inputs.Dropdown(
    choices=list_models(),
    default=names[0] if len(names) > 0 else None,
    label="选择角色",
)

text_language = gr.inputs.Dropdown(
    label="需要合成的语种", choices=["zh", "en", "ja", "auto"], default="auto"
)
...
gr.Interface(
    fn=generate_audio,
    inputs=[text_box, text_language, role_selector],
    outputs=[audio_output, execution_time_output],
    title="文本转语音",
